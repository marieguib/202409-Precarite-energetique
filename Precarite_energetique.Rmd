...
# france_renov29 <- response %>% 
#   select(aggs.value, aggs.metric) %>% 
#   rename("nb_actes_renov" = aggs.metric,
#          "Commune"= aggs.value)

france_renov29 <- response$aggs$metric
france_renov29_code_insee <-response$aggs$results

for (i in seq_along(france_renov29_code_insee)){
  function(x) {
  premier_element <- x[[i]][[1,1]]  
...
premier_element <- sapply(france_renov29$results, function(france_renov29) france_renov29[[1]])
```
