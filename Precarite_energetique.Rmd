...
```{r}
liste_dataframes <- list(beneficiaires_fonds_vert, conso_agriculture, conso_industrie, conso_tertiaire, conso_residentiel, conso_totale, emissions_ges,
...
df_final$beneficiaire_prog380 <- as.factor(df_final$beneficiaire_prog380) # variable à expliquer
...
On paramètre les codes insee comme des identifiants

```{r}
df_final <- df_final %>% 
  column_to_rownames(var="code_insee")
```
...
A revoir si je supprime vraiment 19 lignes.....

# II. Statistiques descriptives

## II.1.Statistiques classiques
...
### II.2.B. Variables quantitatives
...
# II.3. Analyse bivariée / multivariée
...
```{r}
matrixcorr <- cor(df_num)
corrplot(matrixcorr)
```

# II. Constitution de la base de données finale

```{r}
df_final <- df_final %>% 
  select(-c(moyenne_conso_agri, moyenne_conso_indus, moyenne_conso_tertiaire, moyenne_conso_residentiel)) 
