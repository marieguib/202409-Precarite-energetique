...
zonages_prog <- merge(zonages_prog, communes, 
                                  ...
                                  all = TRUE, suffix = c("_zonages", "_com"))

# Mettre NA pour les valeurs non renseignées
zonages_prog$beneficiaire_prog_zonages <- ifelse(is.na(zonages_prog$beneficiaire_prog), NA, zonages_prog$beneficiaire_prog)

zonages_prog <- zonages_prog %>% 
  select(codgeo, beneficiaire_prog_zonages)

zonages_prog$beneficiaire_prog_zonages <- factor(zonages_prog$beneficiaire_prog_zonages)
...
# Mettre NA pour les valeurs non renseignées
loi_littoral$climat <- ifelse(is.na(loi_littoral$CLASSEMENT), NA, loi_littoral$CLASSEMENT)
...
densite7 <- merge(densite7, communes, 
                                  by.x = "codgeo", by.y = "code_commune",
                                  all = TRUE, suffix = c("_densite", "_com"))
...
# Mettre NA pour les valeurs non renseignées
densite7$gridens7 <- ifelse(is.na(densite7$gridens7), NA, densite7$gridens7)

densite7 <- densite7 %>% 
  select(codgeo, gridens7)

densite7$gridens7 <- factor(densite7$gridens7)
...
superficie$superf_choro <- ifelse(is.na(superficie$superf_choro), NA, superficie$superf_choro)
...
Il nous manque une commune donc on fait une jointure avec le fichier des communes pour la récupérer

```{r}
revenu_dispo <- merge(revenu_dispo, communes,
               by.x = "codgeo",  by.y = "code_commune",
               all = TRUE, suffix = c("_revenu_dispo", "_com"))

# Mettre NA pour les valeurs non renseignées
revenu_dispo$med_revenu_dispo <- ifelse(is.na(revenu_dispo$med_disp), NA, revenu_dispo$med_disp)
...
revenu_dispo <- revenu_dispo %>%
 ...
 revenu_dispo <- revenu_dispo %>% 
  select(codgeo, med_revenu_dispo)
...
```{r}
pop <- merge(pop, communes,
               by.x = "codgeo",  by.y = "code_commune",
               all = TRUE, suffix = c("_revenu_dispo", "_com"))

# Remplacer les valeurs NA par 0
pop$pop_commune <- ifelse(is.na(pop$p_pop), NA, pop$p_pop)
