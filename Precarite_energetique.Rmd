...
df_carto_pop <- df_final %>% 
  select(code_insee, p_pop)
  
adresses <- df_carto_pop %>% 
  mutate(Pays = "France")

adresses <- tibble(adresses)

coordonnees_communes <- adresses %>% 
  geocode(postalcode = code_insee, 
          country = Pays,
          method = "osm", 
          progress_bar = T)
...
map <- leaflet(data = df_final) %>%
  addTiles() %>%
  addMarkers(
    ~code_insee,
    popup = ~paste("Code INSEE:", code_insee, "<br>",
                   "Population:", population)
  ) %>%
  setView(lng = 2.35, lat = 48.85, zoom = 5)  # Position et zoom initial de la carte
...
## III.1. Gestion des données déséquilibrées
