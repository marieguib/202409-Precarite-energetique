---
title: "Existe-il des facteurs influençant les demandes de subventions des communes de Bretagne en faveur de la transition écologique ? Si oui, pourquoi ?"
...
```{r}
setdiff(conso_agriculture$code_insee, communes$code_commune)
setdiff(communes$code_commune,conso_agriculture$code_insee)
```

# PROBLEME A GERER : 56049
...
```{r}
setdiff(conso_industrie$code_insee, communes$code_commune)
```
...
```{r}
setdiff(conso_tertiaire$code_insee, communes$code_commune)
```
...
```{r}
setdiff(conso_residentiel$code_insee, communes$code_commune)
```
...
On a bien 1207 communes, on vérifie que ce sont les mêmes que dans notre référentiel

```{r}
setdiff(emissions_ges$code_insee, communes$code_commune)
setdiff(communes$code_commune, emissions_ges$code_insee)
```
...
url_api <- "https://data.ademe.fr/data-fair/api/v1/datasets/sare_tbs/values_agg?field=COMMUNE&metric=sum&metric_field=nb_actes&agg_size=1000&q=29&q_fields=DEPARTEMENT_CODE&size=2&select=codeInsee"

response <- GET(url_api)
data <- fromJSON(content(response, "text"), flatten = TRUE)

france_renov29 <- data$aggs$metric
code_insee <- sapply(data$aggs$results, function(x) x[[1,1]])

france_renov29 <- data.frame(code_insee, nb_actes_france_renov = france_renov29)
```

```{r}
# DPT 35
url_api <-  "https://data.ademe.fr/data-fair/api/v1/datasets/sare_tbs/values_agg?field=COMMUNE&metric=sum&metric_field=nb_actes&agg_size=1000&q=35&q_fields=DEPARTEMENT_CODE&size=2&select=codeInsee"

response <- GET(url_api)
data <- fromJSON(content(response, "text"), flatten = TRUE)

france_renov35 <- data$aggs$metric
code_insee <- sapply(data$aggs$results, function(x) x[[1,1]])

france_renov35 <- data.frame(code_insee, nb_actes_france_renov = france_renov35)
```

```{r}
# DPT 22
url_api <-  "https://data.ademe.fr/data-fair/api/v1/datasets/sare_tbs/values_agg?field=COMMUNE&metric=sum&metric_field=nb_actes&agg_size=1000&q=22&q_fields=DEPARTEMENT_CODE&size=2&select=codeInsee"

response <- GET(url_api)
data <- fromJSON(content(response, "text"), flatten = TRUE)

france_renov22 <- data$aggs$metric
code_insee <- sapply(data$aggs$results, function(x) x[[1,1]])

france_renov22 <- data.frame(code_insee, nb_actes_france_renov = france_renov22)
```

```{r}
# DPT 56
url_api <-  "https://data.ademe.fr/data-fair/api/v1/datasets/sare_tbs/values_agg?field=COMMUNE&metric=sum&metric_field=nb_actes&agg_size=1000&q=56&q_fields=DEPARTEMENT_CODE&size=2&select=codeInsee"

response <- GET(url_api)
data <- fromJSON(content(response, "text"), flatten = TRUE)

france_renov56 <- data$aggs$metric
code_insee <- sapply(data$aggs$results, function(x) x[[1,1]])

france_renov56 <- data.frame(code_insee, nb_actes_france_renov = france_renov56)
```

```{r}
france_renov <- rbind(france_renov35, france_renov22, france_renov29, france_renov56)
...
On a seulement 1185 communes qui ont des actes enregistrés pour France Rénov'. On considère que les autres communes ne présentent aucun acte et on met donc la valeur à 0.

```{r}
france_renov <- merge(communes, france_renov, by.x = "code_commune", by.y = "code_insee", all = T)
head(france_renov)
```

```{r}
france_renov$nb_actes_france_renov <- ifelse(is.na(france_renov$nb_actes_france_renov), 
                                             0,
                                             france_renov$nb_actes_france_renov)

france_renov <- france_renov %>% 
  select(1, 3)
...
Nous allons ajouter les communes ne présentant pas d'écoquartiers. On a seulement 711 communes sur 1207 qui présentent des éco-quartiers.
...
```{r}
FILE_KEY_S3 <- "Sujet_master/Data/X/Logement/part_residences_secondaires.csv"
...
```{r}
residences_secondaires <- residences_secondaires %>% 
  select(1,4) %>% 
  rename("code_insee" = "codgeo", 
         "part_residences_secondaires" = "part_resid2") %>% 
  filter(substr(code_insee, 1, 2) %in% c("22", "29", "35", "56"))

residences_secondaires$part_residences_secondaires <- as.numeric(residences_secondaires$part_residences_secondaires)

head(residences_secondaires)
```

```{r}
setdiff(residences_secondaires$code_insee, communes$code_commune)
setdiff(communes$code_commune, residences_secondaires$code_insee)
```
...
On renomme la colonne qui indique le code insee de la commune.
...
```{r}
setdiff(zonages_prog$code_insee, communes$code_commune)
setdiff(communes$code_commune, zonages_prog$code_insee)
```

#### b) Loi littoral (climat)
...
loi_littoral$climat <- ifelse(is.na(loi_littoral$CLASSEMENT), "Autre", loi_littoral$CLASSEMENT)
...
```{r}
setdiff(loi_littoral$code_insee, communes$code_commune)
setdiff(communes$code_commune, loi_littoral$code_insee)
```
...
```{r}
setdiff(densite7$code_insee, communes$code_commune)
```
...
```{r}
setdiff(superficie$code_insee, communes$code_commune)
```


#### e) Département

Nous allons définir la variable Département pour savoir si le département d'appartenance de la commune joue un rôle dans sa demande de subventions en faveur de la transition écologique

```{r}
departements <- as.data.frame(cbind("code_insee" = communes$code_commune, 
                      "departement" = substr(communes$code_commune, 1,2)))
head(departements)
```

```{r}
setdiff(departements$code_insee, communes$code_commune)
```
...
```{r}
setdiff(gares$code_insee, communes$code_commune)
```
...
```{r}
setdiff(part_deplacement_dom_travail$code_insee, communes$code_commune)
```
...
```{r}
setdiff(revenu_dispo$code_insee, communes$code_commune)
```
...
```{r}
pop <- pop %>% 
  select(codgeo, an, p_pop)

head(pop)
```

```{r}
pop$an <- as.numeric(pop$an)
pop <- pop[order(pop$codgeo, -pop$an), ]
pop <- pop[!duplicated(pop$codgeo),]
...
```{r}
setdiff(pop$code_insee, communes$code_communes)
setdiff(communes$code_communes, pop$code_insee)
```
```{r}
pop %>% 
  filter(code_insee =="56019")
```
...
On ne garde que le code insee et le code de la CSP 
...
```{r}
setdiff(elus$code_insee, communes$code_commune)
setdiff(communes$code_commune, elus$code_insee)
```
...
taux_endettement <- taux_endettement %>% 
  mutate(com_variation_encours_dette_ha_pct = ifelse(com_variation_encours_dette_ha_pct == -9999, 
                                                     NA, com_variation_encours_dette_ha_pct))
```
...
Nous allons importer le fichier qui contient le nombre d'inactifs de 15 à 64 ans en 2020 selon les communes.
...
```{r}
setdiff(nb_inactifs$code_insee, communes$code_commune)
setdiff(communes$code_commune, nb_inactifs$code_insee)
```
...
```{r}
setdiff(dependance_eco$code_insee, communes$code_commune)
setdiff(communes$code_commune, dependance_eco$code_insee)
```
...
```{r}
setdiff(taux_abstention$code_insee, communes$code_commune)
setdiff(communes$code_commune,taux_abstention$code_insee)
```
...
```{r}
setdiff(taux_creation_entreprises$code_insee, communes$code_commune)
setdiff(communes$code_commune, taux_creation_entreprises$code_insee)
```
...
```{r}
setdiff(nb_entreprises$code_insee, communes$code_commune)
setdiff( communes$code_commune, nb_entreprises$code_insee)
```
...
```{r}
setdiff(sportifs_licencies$code_insee, communes$code_commune)
setdiff(communes$code_commune, sportifs_licencies$code_insee)
```
...
```{r}
setdiff(jeunes_sans_diplome$code_insee, communes$code_commune)
setdiff(communes$code_commune, jeunes_sans_diplome$code_insee)
```
...
```{r}
liste_dataframes <- list(beneficiaires_prog_trans_eco, conso_agriculture, conso_industrie, conso_tertiaire, conso_residentiel, conso_totale, emissions_ges, france_renov, friches, communes_eco_quartiers, residences_secondaires, zonages_prog, loi_littoral, 
densite7,  superficie, departements, 
gares, part_deplacement_dom_travail, 
...
df_final$code_insee <- as.factor(df_final$code_insee)
...
df_final$departement <- as.factor(df_final$departement)
...
missing_values_info <- df_final[!complete.cases(df_final), ]

# Afficher les lignes avec des valeurs manquantes
View(missing_values_info)
```

```{r}
# Calculer le nombre de valeurs manquantes par colonne
nb_na_par_colonne <- colSums(is.na(df_final))

# Afficher le résultat
print(nb_na_par_colonne)
```
...
```{r}
# Fonction pour remplacer les valeurs manquantes par la médiane
remplacer_par_mediane <- function(x) {
...
```{r}
# Remplacer les NA par la médiane dans chaque colonne spécifiée
df_final$med_disp <- remplacer_par_mediane(df_final$med_disp)
df_final$com_variation_encours_dette_ha_pct <- remplacer_par_mediane(df_final$com_variation_encours_dette_ha_pct)
df_final$abstention_municipales <- remplacer_par_mediane(df_final$abstention_municipales)
df_final$taux_creation_ent <- remplacer_par_mediane(df_final$taux_creation_ent)
df_final$nb_licencies_sportifs <- remplacer_par_mediane(df_final$nb_licencies_sportifs)
df_final$part_jeunes_sans_diplome <- remplacer_par_mediane(df_final$part_jeunes_sans_diplome)
```
...
Remplacement des données manquantes pour la variable qualitative

```{r}
# Ajouter "Manquant" aux niveaux de la colonne si ce n'est pas déjà fait
df_final$CSP_maire <- factor(df_final$CSP_maire, levels = c(levels(df_final$CSP_maire), "Manquant"))

# Remplacer les NA par "Manquant"
df_final$CSP_maire[is.na(df_final$CSP_maire)] <- "Manquant"
```
...
```{r}
df_final$moyenne_conso_agri <- df_final$moyenne_conso_agri/df_final$p_pop
df_final$moyenne_conso_indus <- df_final$moyenne_conso_indus/df_final$p_pop
df_final$moyenne_conso_tertiaire <- df_final$moyenne_conso_tertiaire/df_final$p_pop
df_final$moyenne_conso_residentiel <- df_final$moyenne_conso_residentiel/df_final$p_pop
df_final$moyenne_conso_totale <- df_final$moyenne_conso_totale/df_final$p_pop


df_final$nb_inactifs <- (df_final$nb_inactifs/df_final$p_pop)*100
df_final$nb_actifs <- (df_final$nb_actifs/df_final$p_pop)*100

df_final$nb_licencies_sportifs <- (df_final$nb_licencies_sportifs/df_final$p_pop)*100
```


### I.3.D. Renommage des colonnes 

```{r}
df_final <- df_final %>% 
  rename("moyenne_conso_agri_hab" = "moyenne_conso_agri", 
         "moyenne_conso_indus_hab" = "moyenne_conso_indus", 
         "moyenne_conso_tertiaire_hab" = "moyenne_conso_tertiaire",
         "moyenne_conso_residentiel_hab" = "moyenne_conso_residentiel",
         "moyenne_conso_totale_hab" = "moyenne_conso_totale", 
         "part_inactifs" = "nb_inactifs", 
         "part_actifs" = "nb_actifs", 
         "part_licencies_sportifs" = "nb_licencies_sportifs")
```
