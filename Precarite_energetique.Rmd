...
ae_fonds_vert_binaire <- ae_fonds_vert %>%
  select(beneficiaire_commune_code) %>% 
  group_by(beneficiaire_commune_code) %>%
  summarize(beneficiaire = 1)

# Conversion pour avoir les mêmes types que dans le fichier communes
ae_fonds_vert_binaire$beneficiaire_commune_code <- as.character(ae_fonds_vert_binaire$beneficiaire_commune_code)
```


Pour pouvoir constituer notre variable à expliquer, il nous faut le reste des communes de Bretagne 
...
On ne conserve que les lignes et colonnes qui nous intéressent 

# VOIR SI C'EST A CONSERVER
--> les siret ne sont pas tous basés en Bretagne donc que faire ? un modèle pour la France entière ? je ne dois conserver que les lignes pour les siret en bretagne ? je ne sais pas trop ???

On filtre d'abord sur la Bretagne
...
communes <- communes %>% 
  ...
  rename(beneficiaire_commune_code = COM)
```

Nous allons maintenant assembler les deux fichiers pour avoir notre variable à expliquer complète

```{r}
beneficiaires_fonds_vert <- left_join(ae_fonds_vert_binaire, communes,
                                      by = "beneficiaire_commune_code",
                                      suffix = c("_ae","_com"))
```
